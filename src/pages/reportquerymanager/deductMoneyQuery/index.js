...
const { Option } = Select;
class deductMoneyQuery extends PureComponent {
  constructor(props) {
    super(props);
    const defaultColumns = [
      ...
      { title: '扣项单号',visible: true, dataIndex: 'business_name', align: 'center', width: 100 },
      { title: '业务类型',visible: true,dataIndex: 'start_time_day', align: 'center', width: 100,},
      { title: '扣减月份', visible: true, dataIndex: 'end_time_day', align: 'center', width: 100 },
      ...
      { title: '合作方ID', visible: true, dataIndex: 'months', align: 'center', width: 100 },
      { title: '合作方名称', visible: true, dataIndex: 'channel4', align: 'center', width: 100 },
      { title: '支付类型', visible: true, dataIndex: 'channel5', align: 'center', width: 100 },
      { title: '设备类型', visible: true, dataIndex: 'channel6', align: 'center', width: 100 },
      { title: '扣项金额', visible: true, dataIndex: 'channel7', align: 'center', width: 100 },
      {
        title: '汇率',
        visible: true,
        dataIndex: 'channel8',
      ...
      { title: '操作人', visible: true, dataIndex: 'channel10', align: 'center', width: 100 },
      { title: '添加时间', visible: true, dataIndex: 'channel11', align: 'center', width: 100 },
      { title: '关联流水ID', visible: true, dataIndex: 'channel12', align: 'center', width: 100 },
      { title: '结算单号', visible: true, dataIndex: 'channel13', align: 'center', width: 100 },
      { title: '结算单状态', visible: true, dataIndex: 'channel14', align: 'center', width: 100 },
      { title: '扣减原因', visible: true, dataIndex: 'channel15', align: 'center', width: 100 },
    ];
    this.state = {
      monthValue: [],
      mode: ['month', 'month'],
  ...
  searchSubmit = e => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      console.log(values);
      dispatch({ type: 'deductmoneyquery/updateList', payload: 1 });
      this.setState({
        visible: true,
      });
    });
  ...
  getListAllKey = () => {
    let listKey = [];
    //获取当前页数据
    const data = this.props.deductmoneyquery.list;
  ...
  render() {
    const { list } = this.props.deductmoneyquery;
...
const Estimate = Form.create()(deductMoneyQuery);
// export default connect()(estimate)
function mapStateToProps(state) {
  return {
    deductmoneyquery: state.deductmoneyquery,
    loading: state.loading.models.deductmoneyquery,
  };
}
