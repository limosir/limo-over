...
const { Option } = Select;
class clearInvoiceQuery extends PureComponent {
  constructor(props) {
    super(props);
  ...
  searchSubmit = e => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      console.log(values);
      dispatch({ type: 'clearinvoicequery/updateList', payload: 1 });
      this.setState({
        visible: true,
      });
    });
  ...
  getListAllKey = () => {
    let listKey = [];
    //获取当前页数据
    const data = this.props.clearinvoicequery.list;
  ...
  render() {
    const { list } = this.props.clearinvoicequery;
...
const Estimate = Form.create()(clearInvoiceQuery);
// export default connect()(estimate)
function mapStateToProps(state) {
  return {
    clearinvoicequery: state.clearinvoicequery,
    loading: state.loading.models.clearinvoicequery,
  };
}
