...
class resetCardTotal extends PureComponent {
  constructor(props) {
    ...
    const defaultColumns = [
      ...
      { title: '期初金额', visible: true, dataIndex: 'end_time_day', align: 'center', width: 100 },
      { title: '本月交易总金额', visible: true, dataIndex: 'months', align: 'center', width: 100 },
      { title: '本月分摊金额', visible: true, dataIndex: 'channel1', align: 'center', width: 100 },
      { title: '未分摊金额', visible: true, dataIndex: 'channel2', align: 'center', width: 100 },
      {
        title: '操作',
        visible: true,
        dataIndex: 'channel3',
        ...
        render: (text, record, index) => (
          <a onClick={() => {this.seeDetail(record, index);}}>查看</a>
        ),
      },
    ];
    this.state = {
  ...
  seeDetail = (record, index) => {
    ...
    router.push({
      pathname: '/reportQueryManager/resetCardTotal/detail',
      query: {
        data: record,
      },
    });
  ...
  searchSubmit = e => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      console.log(values);
      dispatch({ type: 'resetcardtotal/updateList', payload: 1 });
      this.setState({
        visible: true,
      });
    });
  ...
  getListAllKey = () => {
    let listKey = [];
    //获取当前页数据
    const data = this.props.resetcardtotal.list;
  ...
  render() {
    const { list } = this.props.resetcardtotal;
...
function mapStateToProps(state) {
  return {
    resetcardtotal: state.resetcardtotal,
    loading: state.loading.models.resetcardtotal,
  };
}
