...
const { Option } = Select;
class incomeInvoiceQuery extends PureComponent {
  constructor(props) {
    super(props);
    const defaultColumns = [
      ...
      { title: '结算单号',visible: true, dataIndex: 'business_name', align: 'center', width: 100 },
      { title: '合作方ID',visible: true,dataIndex: 'start_time_day', align: 'center', width: 100,},
      { title: '合作方名称', visible: true, dataIndex: 'end_time_day', align: 'center', width: 100 },
      { title: '销方纳税人识别号', visible: true, dataIndex: 'channel1', align: 'center', width: 100 },
      { title: '发票代码', visible: true, dataIndex: 'channel2', align: 'center', width: 100 },
      { title: '发票号码', visible: true, dataIndex: 'channel3', align: 'center', width: 100 },
      { title: '开票内容', visible: true, dataIndex: 'months', align: 'center', width: 100 },
      { title: '发票类型', visible: true, dataIndex: 'channel4', align: 'center', width: 100 },
      { title: '发票税率', visible: true, dataIndex: 'channel5', align: 'center', width: 100 },
      { title: '发票金额', visible: true, dataIndex: 'channel6', align: 'center', width: 100 },
      { title: '发票税额', visible: true, dataIndex: 'channel7', align: 'center', width: 100 },
      { title: '发票价税合计',visible: true,dataIndex: 'channel8',align: 'center',width: 150, },
      { title: '开票日期',visible: true,dataIndex: 'channel9',align: 'center',width: 160, },
      { title: '审核状态', visible: true, dataIndex: 'channel10', align: 'center', width: 100 },
      { title: '发票状态', visible: true, dataIndex: 'channel11', align: 'center', width: 100 },
      { title: '发票审核时间', visible: true, dataIndex: 'channel12', align: 'center', width: 100 },
      { title: '结算状态', visible: true, dataIndex: 'channel13', align: 'center', width: 100 },
      { title: '购方名称', visible: true, dataIndex: 'channel14', align: 'center', width: 100 },
      { title: '购方纳税人识别号', visible: true, dataIndex: 'channel15', align: 'center', width: 100 },
      { title: '备注', visible: true, dataIndex: 'channel16', align: 'center', width: 100 },
    ];
    this.state = {
      monthValue: [],
      mode: ['month', 'month'],
  ...
  searchSubmit = e => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      console.log(values);
      dispatch({ type: 'incomeinvoicequery/updateList', payload: 1 });
      this.setState({
        visible: true,
      });
    });
  ...
  getListAllKey = () => {
    let listKey = [];
    //获取当前页数据
    const data = this.props.incomeinvoicequery.list;
  ...
  render() {
    const { list } = this.props.incomeinvoicequery;
...
const Estimate = Form.create()(incomeInvoiceQuery);
// export default connect()(estimate)
function mapStateToProps(state) {
  return {
    incomeinvoicequery: state.incomeinvoicequery,
    loading: state.loading.models.incomeinvoicequery,
  };
}
