...
const { Option } = Select;
class businessTypeManager extends PureComponent {
  constructor(props) {
    super(props);
    const defaultColumns = [
      ...
      { title: '操作人', visible: true, dataIndex: 'channel3', align: 'center', width: 100 },
      {
        title: '操作原因',
        visible: true,
        dataIndex: 'channel4',
  ...
  searchSubmit = e => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      console.log(values);
      dispatch({ type: 'businesstypemanager/updateList', payload: 1 });
      this.setState({
        visible: true,
      });
    });
  ...
  getListAllKey = () => {
    let listKey = [];
    //获取当前页数据
    const data = this.props.businesstypemanager.list;
  ...
  render() {
    const { list } = this.props.businesstypemanager;
        ...
        <Card bordered={false}>
          ...
          <div className={styles.contentbox}>
            ...
            <div className="pos">
              ...
              <Button type="primary">批量修改业务类型</Button>
            </div>
          </div>
        </Card>
      </div>
...
const Estimate = Form.create()(businessTypeManager);
// export default connect()(estimate)
function mapStateToProps(state) {
  return {
    businesstypemanager: state.businesstypemanager,
    loading: state.loading.models.businesstypemanager,
  };
}
