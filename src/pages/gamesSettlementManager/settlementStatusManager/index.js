...
const { Option } = Select;
class settlementStatusManager extends PureComponent {
  constructor(props) {
    super(props);
    const defaultColumns = [
      ...
      { title: '业务类型', visible: true, dataIndex: 'pay_flow_uuid', align: 'center', width: 100 },
      { title: '支付类型', visible: true, dataIndex: 'channel5', align: 'center', width: 100 },
      { title: '合作方ID', visible: true, dataIndex: 'business_name', align: 'center', width: 100 },
      { title: '合作方名称', visible: true, dataIndex: 'start_time_day', align: 'center', width: 100 },
      { title: 'APPID', visible: true, dataIndex: 'end_time_day', align: 'center', width: 100 },
      { title: '游戏ID', visible: true, dataIndex: 'months', align: 'center', width: 100 },
      { title: '游戏名称', visible: true, dataIndex: 'channel1', align: 'center', width: 100 },
      { title: '操作时间', visible: true, dataIndex: 'channel2', align: 'center', width: 100 },
      { title: '结算暂停起始月份', visible: true, dataIndex: 'channel3', align: 'center', width: 100 },
      {
        title: '结算暂停结束月份',
        visible: true,
        dataIndex: 'channel4',
  ...
  searchSubmit = e => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      console.log(values);
      dispatch({ type: 'settlementstatusmanager/updateList', payload: 1 });
      this.setState({
        visible: true,
      });
    });
  ...
  getListAllKey = () => {
    let listKey = [];
    //获取当前页数据
    const data = this.props.settlementstatusmanager.list;
  ...
  render() {
    const { list } = this.props.settlementstatusmanager;
        ...
        <Card bordered={false}>
          ...
          <div className={styles.contentbox}>
            ...
            <div className="pos">
              ...
              <Button type="primary" className="mgr10">恢复结算</Button>
              <Button type="primary">暂停结算</Button>
            </div>
          </div>
        </Card>
      </div>
...
const Estimate = Form.create()(settlementStatusManager);
// export default connect()(estimate)
function mapStateToProps(state) {
  return {
    settlementstatusmanager: state.settlementstatusmanager,
    loading: state.loading.models.settlementstatusmanager,
  };
}
