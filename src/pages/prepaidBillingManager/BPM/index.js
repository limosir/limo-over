...
const { Option } = Select;
class prepaidbillingbpm extends PureComponent {
  constructor(props) {
    super(props);
  ...
  seeDetail = (record, index) => {
    ...
    router.push({
      pathname: '/prepaidBillingManager/BPM/detail',
      query: {
        data: record,
      },
    });
  ...
  searchSubmit = e => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      console.log(values);
      dispatch({ type: 'prepaidbillingbpm/updateList', payload: 1 });
      this.setState({
        visible: true,
      });
    });
  ...
  getListAllKey = () => {
    let listKey = [];
    //获取当前页数据
    const data = this.props.prepaidbillingbpm.list;
  ...
  render() {
    const { list } = this.props.prepaidbillingbpm;
...
const Estimate = Form.create()(prepaidbillingbpm);
// export default connect()(estimate)
function mapStateToProps(state) {
  return {
    prepaidbillingbpm: state.prepaidbillingbpm,
    loading: state.loading.models.prepaidbillingbpm,
  };
}
