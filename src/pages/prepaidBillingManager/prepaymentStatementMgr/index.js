...
const { Option } = Select;
class prepaymentStatementMgr extends PureComponent {
  constructor(props) {
    super(props);
  ...
  searchSubmit = e => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      console.log(values);
      dispatch({ type: 'prepaymentstatement/updateList', payload: 1 });
      this.setState({
        visible: true,
      });
    });
  ...
  getListAllKey = () => {
    let listKey = [];
    //获取当前页数据
    const data = this.props.prepaymentstatement.list;
  ...
  render() {
    const { list } = this.props.prepaymentstatement;
...
const Estimate = Form.create()(prepaymentStatementMgr);
// export default connect()(estimate)
function mapStateToProps(state) {
  return {
    prepaymentstatement: state.prepaymentstatement,
    loading: state.loading.models.prepaymentstatement,
  };
}
