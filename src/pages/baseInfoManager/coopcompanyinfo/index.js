...
class Coopcompanyinfo extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      selectedRowKeys: [],
    ...
    const columns = [
      ...
      { title: '币种', visible: true, dataIndex: 'end_time_day', align: 'center' },
      {
        ...
        render: (text, record) => {
          return (
            <div>
              <a className="mgr" onClick={() => this.goAdd('edit', text, record)}>
                编辑
              </a>
              <a className="red" onClick={() => this.deleteItem(text, record)}>
                删除
              </a>
            </div>
          );
          ...
          <div className={styles.search} id="search">
            <Form layout="inline" onSubmit={this.searchSubmit}>
              <Row className="mgb10">
                ...
                <Col span={6}>
                  <FormItem>
                    <Button type="primary" className="mgr10" htmlType="submit">
                      查询
                    </Button>
                    <Button type="primary" className="mgr20" onClick={this.resetForm}>
                      重置
                    </Button>
                    <Button type="primary" className="mgr10" onClick={this.goAdd}>
                      新增企业
                    </Button>
                  </FormItem>
                </Col>
...
function mapStateToProps(state) {
  return {
    ...
    coopCompanyInfo: state.coopCompanyInfo,
    loading: state.loading.models.coopCompanyInfo,
  };
}
export default connect(mapStateToProps)(Form.create()(Coopcompanyinfo));
